...
### Added
- Add audio visual reporting system under `audio_enhancer/reports/` using Strategy, Builder, Factory, and Facade patterns.
- Implement `SpectrogramReport` to compare original and processed spectrograms.
- Implement `WaveformReport` to compare amplitude waveforms.
- Implement `FrequencySpectrumReport` to compare FFT frequency spectra.
- Integrate report generation in CLI (`--generate-report` flag) and the `AudioEnhancer` facade.
- Add comprehensive documentation, tests, and example script.
